...
<script type="text/javascript"> 
	$(document).ready(function(){ 
		...
		$("#update").on("click", function(e){ 
			e.preventDefault(); 
			fn_openStudentBoardUpdate(); 
			});
		
		$("a[name='file']").on("click", function(e){ //파일 이름 
			e.preventDefault(); 
		fn_downloadStudentFile($(this)); 
		});
	...
	function fn_openStudentBoardUpdate(){ 
		var STU_NUM = "${map.STU_NUM}"; 
		var comSubmit = new ComSubmit("frm");
		comSubmit.setUrl("<c:url value='/studentboard/openStudentBoardUpdate' />");
		comSubmit.addParam("STU_NUM", STU_NUM); 
		comSubmit.submit(); 
		}
	function fn_downloadStudentFile(obj){ 
		var STU_NUM = obj.student().find("#STU_NUM").val(); 
		var comSubmit = new ComSubmit(); 
		comSubmit.setUrl("<c:url value='/common/downloadStudentFile' />"); 
		comSubmit.addParam("STU_NUM", STU_NUM); 
		comSubmit.submit(); 
		}
	</script>
			...
			<div class="tbl_type_01">
				<table>
					...
					<tbody>
						...
						<input type="hidden" id="STU_NUM" value="${row.STU_NUM }"> 
