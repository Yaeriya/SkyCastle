...
<script type="text/javascript"> 
	$(document).ready(function(){ 
		...
		$("#update").on("click", function(e){ 
			e.preventDefault(); 
			fn_openFreeBoardUpdate(); 
			});
		
		$("a[name='file']").on("click", function(e){ //파일 이름 
			e.preventDefault(); 
		fn_downloadFreeFile($(this)); 
		});
	...
	function fn_openFreeBoardUpdate(){ 
		var FREE_NUM = "${map.FREE_NUM}"; 
		var comSubmit = new ComSubmit(); 
		comSubmit.setUrl("<c:url value='/freeboard/boardFreeUpdate' />"); 
		comSubmit.addParam("FREE_NUM", FREE_NUM); 
		comSubmit.submit(); 
		}
	function fn_downloadFreeFile(obj){ 
		var FREE_NUM = obj.parent().find("#FREE_NUM").val(); 
		var comSubmit = new ComSubmit(); 
		comSubmit.setUrl("<c:url value='/common/downloadFreeFile' />"); 
		comSubmit.addParam("FREE_NUM", FREE_NUM); 
		comSubmit.submit(); 
		}
			...
			<div class="tbl_type_01">
				<table>
					...
					<tbody>
						...
						<input type="hidden" id="FREE_NUM" value="${row.FREE_NUM }"> 
