							...
							<c:choose>
								<c:when test="${fn:length(list) > 0}">
									<c:forEach items="${list }" var="row">
										<tr>
											<td>${row.PAR_NUM }</td>
											<td class="title"><a href="#this" name="PAR_TITLE">${row.PAR_TITLE }</a>
												<input type="hidden" id="PAR_NUM" value="${row.PAR_NUM }"></td>
											<td>${row.MB_NICK}</td>
											<td>${row.PAR_DATE}</td>
											<td>${row.PAR_HIT}</td>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#write").on("click", function(e) {//글쓰기 버튼
				e.preventDefault();
				fn_openParentBoardWrite();
			});

			$("a[name='PAR_TITLE']").on("click", function(e) { //제목
				e.preventDefault();
				fn_openParentBoardDetail($(this));
			});
		});
		function fn_openParentBoardWrite() {
			var comSubmit = new ComSubmit();
			comSubmit
					.setUrl("<c:url value='/parentboard/openParentBoardWrite' />");
			comSubmit.submit();
		}
		function fn_openParentBoardDetail(obj) {
			var comSubmit = new ComSubmit();
			comSubmit
					.setUrl("<c:url value='/parentboard/openParentBoardDetail' />");
			comSubmit.addParam("PAR_NUM", obj.parent().find("#PAR_NUM").val());
			comSubmit.submit();
		}
	</script>
