...
<script type="text/javascript"> 
	$(document).ready(function(){ 
		...
		$("#update").on("click", function(e){ 
			e.preventDefault(); 
			fn_openParentBoardUpdate(); 
			});
		
		$("a[name='file']").on("click", function(e){ //파일 이름 
			e.preventDefault(); 
		fn_downloadParentFile($(this)); 
		});
	...
	function fn_openParentBoardUpdate(){ 
		var PAR_NUM = "${map.PAR_NUM}"; 
		var comSubmit = new ComSubmit("frm");
		comSubmit.setUrl("<c:url value='/parentboard/openParentBoardUpdate' />");
		comSubmit.addParam("PAR_NUM", PAR_NUM); 
		comSubmit.submit(); 
		}
	function fn_downloadParentFile(obj){ 
		var PAR_NUM = obj.parent().find("#PAR_NUM").val(); 
		var comSubmit = new ComSubmit(); 
		comSubmit.setUrl("<c:url value='/common/downloadParentFile' />"); 
		comSubmit.addParam("PAR_NUM", PAR_NUM); 
		comSubmit.submit(); 
		}
	</script>
			...
			<div class="tbl_type_01">
				<table>
					...
					<tbody>
						...
						<input type="hidden" id="PAR_NUM" value="${row.PAR_NUM }"> 
